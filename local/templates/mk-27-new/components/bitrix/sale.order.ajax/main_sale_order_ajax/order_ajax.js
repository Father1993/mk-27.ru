...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		locationsCompletion: function()
		{
			...
			/**/ // Активируем редактирование блоков "Регион" и "Пользователь"
		...
		editSection: function(section)
		{
			...
			/**/ // Убираем действия при клике на заголовки
			/*BX.unbindAll(titleNode);
			if (this.result.SHOW_AUTH)
			{
...
*/
		...
		createBasketItemContent: function(data)
		{
			...
			if (this.params.HIDE_DETAIL_PAGE_URL !== 'Y' && data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length)
			{
				/**/ // Убираем ссылку на товар 
				titleHtml = '<span class="item-title">' + titleHtml + '</span>';
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'form-group bx-soa-location-input-container original-location'},
			...
			// this.getZipLocationInput(node); /**/ // Закрыли вывод индекса в блоке местоположений
		...
		editPropsItems: function(propsNode)
		{
			...
			this.getZipLocationInput(propsItemsContainer); /**/ // Это мы добавили для вывода индекса в личные данные
