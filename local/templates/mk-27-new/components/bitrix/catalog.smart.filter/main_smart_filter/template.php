...
<div class="filter-show-button">Показать фильтры</div>
			...
			<div class="row">
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
                    ...
                    foreach ($arItem["VALUES"] as $key => $sortkey) {
                        if (is_numeric($sortkey["VALUE"][0])) {
                            $string = htmlentities($sortkey["VALUE"], null, 'utf-8');
                            $string = str_replace(",", ".", $string);
                            $string = str_replace("&nbsp;", "", $string);
                            $sortkey["VALUE"] = html_entity_decode($string);
                        }
											...
											<div class="radio">
												<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label" for="<? echo $ar["CONTROL_ID"] ?>">
													<span class="bx-filter-input-checkbox <? echo $ar["DISABLED"] ? 'disabled': '' ?>">
														...
														<span class="bx-filter-param-text" title="<?=$ar["VALUE"];?>"><?=$ar["VALUE"];?><?
														if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):
															?>&nbsp;<span class="filter-count">(<span data-role="count_<?=$ar["CONTROL_ID"]?>"><? echo $ar["ELEMENT_COUNT"]; ?></span>)</span><?
														endif;?></span>
													</span>
												</label>
											</div>
