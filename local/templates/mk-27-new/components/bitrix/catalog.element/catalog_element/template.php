...
if ($arResult["PHOTOS"])
{
    $oneClickProductPhoto = CFile::ResizeImageGet($arResult["PHOTOS"][0]["ID"], array("width"=>"250", "height"=>"250"), BX_RESIZE_IMAGE_PROPORTIONAL, true);
}
else 
{
    $oneClickProductPhoto = CFile::ResizeImageGet($arResult["DETAIL_PICTURE"]["ID"], array("width"=>"250", "height"=>"250"), BX_RESIZE_IMAGE_PROPORTIONAL, true);
}
$oneClickUrl = ((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
$oneClickUrl = explode('?', $oneClickUrl);
$oneClickUrl = $oneClickUrl[0];
...
<div class="one-click-popup">
	...
	<form action="" class="one-click-form" method="post" onsubmit="oneClickBuy(); return false;">
    	<input type="text" class="one-click-field" required><?php // Антибот ?>
    	<input type="text" class="one-click-name" placeholder="Имя" required>
    	<input type="phone" class="one-click-phone" placeholder="Телефон" required>
    	<input type="hidden" class="one-click-product-name" value="<?=$name?>" required>
    	<input type="hidden" class="one-click-product-price" value="<?=$arResult["ITEM_PRICES"][0]["PRICE"]?>" required>
    	<input type="hidden" class="one-click-product-link" value="<?=$oneClickUrl?>" required>
    	<input type="hidden" class="one-click-product-photo" value="<?=$oneClickProductPhoto["src"]?>" required>
    	<input type="submit" class="send" value="Купить">
	</form>

	<div class="one-clock-success">Ваша заявка принята, наш менеджер свяжется с вами в ближайшее время.</div>
</div>

<div class="product">
	
	<div class="container main-emelent-block no-padding-container">
	
		<div class="row">
        
        	<div class="col-12 col-md-6 image-block">
        	
        	<h2 class="mobile-title"><?=$name?></h2>
        		...
        		<?php if ($arResult["PHOTOS"]): ?>
                	...
                	<div class="photos">
                    	<?php foreach ($arResult["PHOTOS"] as $photo): ?>
                        	...
                        	<?php 
                        	if (empty($photo["ID"])) {
                        	    $photoLargeSrc["src"] = SITE_TEMPLATE_PATH . "/assets/images/placeholder.jpg";
                        	    $photoSmallSrc["src"] = SITE_TEMPLATE_PATH . "/assets/images/placeholder.jpg";
                        	} else {
                        	    $photoLargeSrc = CFile::ResizeImageGet($photo["ID"], array("width"=>"1920", "height"=>"1024"), BX_RESIZE_IMAGE_PROPORTIONAL, true);
                        	    $photoSmallSrc = CFile::ResizeImageGet($photo["ID"], array("width"=>"465", "height"=>"400"), BX_RESIZE_IMAGE_PROPORTIONAL, true);
                        	}
                        	?>
                        	
                    		<a data-fancybox="gallery" href="<?=$photoLargeSrc["src"]?>" data-hash="<?=$photo["ID"]?>">
                				<div class="image" style="background-image: url('<?=$photoSmallSrc["src"]?>')"></div>
            				</a>
                	...
                	<?php if (count($arResult["PHOTOS"]) > 1): ?>
                    	...
                    	<div class="photos-nav owl-carousel"> 
                    		
                    		<?php foreach ($arResult["PHOTOS"] as $photo): ?>
                    		
                    			<?php $photoNavSrc = CFile::ResizeImageGet($photo["ID"], array("width"=>"80", "height"=>"80"), BX_RESIZE_IMAGE_PROPORTIONAL, true); ?>
                        		
                        		<a href="#<?=$photo["ID"]?>">
                        			<div class="nav-point" style="background-image: url('<?=$photoNavSrc["src"]?>')"></div>
                        		</a>
            	...
            	<?php else: ?>
            		...
            		<?php $photoSrc = CFile::ResizeImageGet($arResult["DETAIL_PICTURE"]["ID"], array("width"=>"465", "height"=>"400"), BX_RESIZE_IMAGE_PROPORTIONAL, true); ?>

                	<div class="product-photo">
                		<a data-fancybox="gallery" href="<?=$arResult["DETAIL_PICTURE"]["SRC"]?>">
            				<div class="image" style="background-image: url('<?=$photoSrc["src"]?>')"></div>
        				</a>
                	</div>
        	...
        	<div class="col-12 col-md-6 info">
        		...
        		<?php if ($USER->IsAdmin()): ?>
            		<div class="quantity">
            			Остаток: <strong><?=$arResult["PRODUCT"]["QUANTITY"]?></strong>
            		</div>
        		<?php endif; ?>
        	
            	<div class="container no-padding-container info-container">
            		<div class="row">
            		
            		<?php if ($arResult["ITEM_PRICES"][0]["PRICE"] > 0): ?>
            			...
            			<div class="col-12 col-lg-6 col-xl-5">
                    		...
                    		<div class="article">
                    			...
                    			<span class="article-value"><?=$arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]?></span>
                    		</div>
                    		<?php endif; ?>
                    		...
                    		<?php else: ?>
                    			<div class="in-stock out-of-stock">Нет в наличии</div>
                			<?php endif; ?>
            
                    		<?php */ ?>
                    		
                    		<div class="price"><?=number_format($arResult["ITEM_PRICES"][0]["PRICE"], 2, '.', ' ');?><i class="fa fa-rub" aria-hidden="true"></i></div>
            				...
            				<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" class="basket-action-block">
            					<?
            					if ($showAddBtn)
            					{
            						?>
            					...
            					<div class="favorite" data-item="<?=$arResult["ID"]?>">
            						<div class="hint">
            			...
            			<?php else: ?>
							...
							<div class="product-order-popup">
								...
								<div class="product-order-popup__title">Оставьте заявку<br>и мы свяжемся с вами</div>
								
								<form action="" class="product-order-popup__form" method="POST" onsubmit="sendProductOrder(); return false;">
									<input type="hidden" name="PRODUCT_XML_ID" value="<?=$arResult["XML_ID"]?>">
									<input type="text" class="antibot" name="ANTIBOT" required>
            			...
            			<div class="col-12 col-lg-6 col-xl-5">
            				<div class="price" onclick="showProductOrderPopup(); return false;">Под заказ</div>
            				<div class="button-product-order" onclick="showProductOrderPopup(); return false;">Заказать</div>
        				</div>
            			<?php endif; ?>
            			...
            			<div class="col-12 col-lg-6 col-xl-7">
            				...
            				<div class="additional-text">
								<?$APPLICATION->IncludeComponent("bitrix:main.include","",Array("AREA_FILE_SHOW" => "file",
                                    "PATH" => SITE_TEMPLATE_PATH . "/components/bitrix/catalog.element/catalog_element/include/additional_text_1.php"
                                ));?>
            				</div>
...
<div class="container no-padding-container second-block">
	<div class="row">
		<div class="col-12 col-md-6 description">
		
			<?php if ($arResult["DETAIL_TEXT"]): ?>
    			<div class="description-title">Описание</div>
    			<div class="description"><?=str_replace("&nbsp;", " ", preg_replace('/\s+/', ' ', $arResult["DETAIL_TEXT"]))?></div>
			<?php endif; ?>
		...
		<div class="col-12 col-md-6">
    		...
    		<?php $i = 0; ?>
			<?php foreach ($arResult["DISPLAY_PROPERTIES"] as $key => $val):?>
