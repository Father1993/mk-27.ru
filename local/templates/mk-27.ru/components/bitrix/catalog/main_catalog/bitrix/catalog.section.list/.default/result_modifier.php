...
foreach ($arResult["SECTIONS"] as $key => $section)
{
    ...
    {
        $res = CIBlockElement::GetList(Array(), Array("SECTION_ID" => $section["ID"]), false, false, Array("ID", "NAME", "DETAIL_PICTURE", "CATALOG_PRICE_2", "CATALOG_QUANTITY"));
    }
    $output = false;
    while ($arItem = $res->fetch())
    {
        if ($arItem["DETAIL_PICTURE"] && $arItem["CATALOG_PRICE_2"] && $arItem["CATALOG_QUANTITY"])
        {
            $output = true;
            break;
        }
