...
<?php 
// Так как убрали "/catalog/" из URL каталога, проверяем ссылки на наличие контента, иначе 404. 
...
// Нужно убрать фильтр для разделов не последнего уровня т.к. в них нет товаров
/*
// Получаем ID раздела
$rsSections = CIBlockSection::GetList(
    array(),
    array("IBLOCK_ID" => "8", "=CODE" => $arResult["VARIABLES"]["SECTION_CODE"]),
    false,
    array("ID")
);
...
// Выбираем подразделы текущего раздела
...
if ($arParentSection = $rsParentSection->GetNext())
{
    ...
    while ($arSect = $rsSect->GetNext())
    {
        if ($arSect)
        {
            // Если есть подразделы, то отмечаем это
            $isFilter = false;
            $isSidebar = false;
        }
    }
...
?>
<?php global $USER; ?>
<h1 class="main-section-title"><?$APPLICATION->ShowTitle(false);?><?php if ($USER->IsAdmin()) echo " (ID: ".$arResult["VARIABLES"]["SECTION_ID"].")";?></h1>
<?php
...
if ($isFilter || $isSidebar): ?>
	<div class="col-lg-3 smart-filter-col <?=(isset($arParams['FILTER_HIDE_ON_MOBILE']) && $arParams['FILTER_HIDE_ON_MOBILE'] === 'Y' ? ' hidden-xs' : '')?>">
		<? if ($isFilter): ?>
...
<?endif?>
<div class="<?=(($isFilter || $isSidebar) ? "col-lg-9" : "col-xs-12")?>">
	<div class="row">
		<div class="col-xs-12">
			<?
		...
		<div class="col-xs-12 catalog-section-col">
			<?
...
*/
			...
			//{
    			...
    			if ($arParams["CITY_CODE"] !== "ysl") {
    			    ...
    			    if ($priceTypeYsl == "Сахалин РОЗНИЦА") { // header.php
    			        $arrFilter["!CATALOG_PRICE_14"] = false;
    			    } elseif ($priceTypeYsl == "Сахалин ОПТ") {
    			        $arrFilter["!CATALOG_PRICE_2"] = false;
    			    }
			...
			if (!empty($arRecomData))
			{
				if (!isset($arParams['USE_BIG_DATA']) || $arParams['USE_BIG_DATA'] != 'N')
				{
					...
					<div class="col-xs-12" data-entity="parent-container">
						...
						//if (!$USER->IsAdmin())
						//{
						    GLOBAL $arRecomendedFilter;
						    //$arRecomendedFilter['!DETAIL_PICTURE'] = false;
						    //$arRecomendedFilter[">CATALOG_QUANTITY"] = 0;
						    if ($arParams["CITY_CODE"] !== "ysl") {
						        $arRecomendedFilter["!CATALOG_PRICE_1"] = false;
						    } else {
						        
						        if ($priceTypeYsl == "Сахалин РОЗНИЦА") { // header.php
						            $arRecomendedFilter["!CATALOG_PRICE_14"] = false;
						        } elseif ($priceTypeYsl == "Сахалин ОПТ") {
						            $arRecomendedFilter["!CATALOG_PRICE_2"] = false;
						        }
						...
						$APPLICATION->IncludeComponent(
							...
							array(
								...
								'BACKGROUND_IMAGE' => '',
								'DISABLE_INIT_JS_IN_COMPONENT' => (isset($arParams['DISABLE_INIT_JS_IN_COMPONENT']) ? $arParams['DISABLE_INIT_JS_IN_COMPONENT'] : ''),
							    "FILTER_NAME" => "arRecomendedFilter",
							),
							$component
						);
						?>
