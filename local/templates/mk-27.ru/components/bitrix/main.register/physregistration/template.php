...
// Флаг для отключения капчи в режиме разработки
// Установите в false перед деплоем в продакшн
$isDevMode = true;
...
<?else:?>
<script>
// В режиме разработки автоматически активируем кнопку отправки формы
document.addEventListener('DOMContentLoaded', function() {
  if (document.getElementById('register_submit_button')) {
    document.getElementById('register_submit_button').disabled = false;

    // Скрываем уведомление о необходимости подтверждения
    var notice = document.querySelector('.submit-disable-notice');
    if (notice) notice.style.display = 'none';
  }
});
</script>
<?endif;?>
<div class="bx-auth-reg legal-entity">
...
?>
  ...
  <?if($arResult["SHOW_SMS_FIELD"] == true):?>AUTH_REGISTER
    ...
    <table>
      <tbody>
        <tr>
          ...
          <td><input size="30" type="text" name="SMS_CODE" value="<?=htmlspecialcharsbx($arResult["SMS_CODE"])?>"
              autocomplete="off" /></td>
        </tr>
      </tbody>
      <tfoot>
        <tr>
          <td></td>
          <td><input type="submit" name="code_submit_button" value="<?echo GetMessage(" main_register_sms_send")?>" />
          </td>
        </tr>
      </tfoot>
    </table>
...
?>
	...
	switch ($FIELD)
	{
			...
			?>
              <div class="bx-authform-formgroup-container">
                ...
                <div class="bx-authform-input-container">
                  <input size="30" type="password" required name="REGISTER[<?=$FIELD?>]"
		...
		case "WORK_COUNTRY":
			...
			{
				?>
                <option value="<?=$value?>" <?if ($value==$arResult["VALUES"][$FIELD]):?> selected="selected"
		...
		default:
			...
			?>
              ...
              <div class="bx-authform-formgroup-container">
                ...
                <div class="bx-authform-input-container">
                  <input size="30" type="text" required name="REGISTER[<?=$FIELD?>]"
    ...
    <div class="bx-authform-formgroup-container">
      <?if(!$isDevMode):?>
      <div class="g-recaptcha" data-sitekey="<?=$publicKey?>" data-callback="onRecaptchaSuccesSs"></div>
      <?else:?>
      <!-- Капча отключена в режиме разработки -->
      <?endif;?>
    </div>
    <div class="bx-authform-formgroup-container">
      <input <?if(!$isDevMode):?>disabled
      <?endif;?> type="submit" class="btn btn-primary" id="register_submit_button" name="register_submit_button"
