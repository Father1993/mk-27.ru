...
<?else:?>
	<input type="text" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" size="40" placeholder="Ключевые слова" />
<?endif;?>
	...
	<?if($arParams["DISPLAY_TOP_PAGER"] != "N") echo $arResult["NAV_STRING"]?>
			<div class="sections">
            	...
            	<?php foreach($arResult["SEARCH"] as $arItem): ?>
            		<?php if (empty($arResult["ADVANCED"][$arItem["ITEM_ID"]]["IS_PRODUCT"])):?>
            			<?php if ($i == 0): ?>
            				<div class="section-title">Разделы:</div>
            			<?php endif; ?>
            			<?php $i++; ?>
            			<a class="section-link" href="<?=$arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a>
            		<?php endif; ?>
        		<?php endforeach; ?>
        		
    		</div>
	
	<div class="item-title">Товары:</div>
	...
	<div class="container">
		<div class="row">
        	...
        	<?php foreach($arResult["SEARCH"] as $arItem): ?>
        		...
        		<?php if (($arResult["ADVANCED"][$arItem["ITEM_ID"]]["IS_PRODUCT"] === TRUE)):?>
            		<div class="col-6 col-sm-4 col-md-3 col-lg-2 item">
                		
                		<?php if (empty($arResult["ADVANCED"][$arItem["ITEM_ID"]]["DETAIL_PICTURE"])) $arResult["ADVANCED"][$arItem["ITEM_ID"]]["DETAIL_PICTURE"] = SITE_TEMPLATE_PATH . "/assets/images/placeholder.jpg"; ?>
                		
                		<a class="image" href="<?=$arItem["URL"]?>">
                			<img src="<?=$arResult["ADVANCED"][$arItem["ITEM_ID"]]["DETAIL_PICTURE"]?>">
                		</a>
                		
                		<a class="title-link" href="<?=$arItem["URL"]?>"><?echo $arItem["TITLE_FORMATED"]?></a>
                		
                		<div class="price">
                			<?php if ($arResult["ADVANCED"][$arItem["ITEM_ID"]]["PRICE"] > 0): ?>
                				<?=number_format($arResult["ADVANCED"][$arItem["ITEM_ID"]]["PRICE"], 2, '.', ' ');?><i class="fa fa-rub"></i>
            				<?php else: ?>
        						Под заказ
            				<?php endif;?>
                		</div>

                		<?php /* ?><p><?echo $arItem["BODY_FORMATED"]?></p><?php */ ?>
