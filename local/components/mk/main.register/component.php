...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["register_submit_button"] <> '' && !$USER->IsAuthorized())
{
	...
	else // if there's no any errors - create user
	{
		...
		if(!empty($arResult["VALUES"]["UF_CONTACT_PERSON"])) {
		    $arUSERFio = explode(" ",$arResult["VALUES"]["UF_CONTACT_PERSON"]);
        }
		
		$arResult["VALUES"]["LAST_NAME"] = $arUSERFio[0];
		$arResult["VALUES"]["NAME"] = $arUSERFio[1];
		$arResult["VALUES"]["SECOND_NAME"] = $arUSERFio[2];
...
//если пользователь - юр лицо, создаем новый профиль пользователя

if($arResult["VALUES"]["UF_ISURFACE"] == "Да") {
    $payerType = 2;
} else {
    $payerType = 1;
}
...
if($payerType == 2) {
    ...
    $profileId = CSaleOrderUserProps::Add($profileFields);
    if ($profileId) {
        //echo "Профиль успешно создан, ID: " . $profileId;
        // Параметры свойств профиля, включая дополнительные поля
        $props = [
            "CONTACT_PERSON" => $arResult["VALUES"]["UF_FIO"],
            "ADDRESS" => $arResult["VALUES"]["UF_DELIVERY_ADDR"],
            "EMAIL" => $arResult["VALUES"]["EMAIL"],
            "COMPANY" => $arResult["VALUES"]["UF_ORGNAME"],
            "INN" => $arResult["VALUES"]["UF_ORGINN"],
            "KPP" => $arResult["VALUES"]["UF_KPPORG"],
            "COMPANY_ADR" => $arResult["VALUES"]["UF_URADDRESS"],
            "PHONE" => $arResult["VALUES"]["UF_HEAD_PHONE"],
        ];

        foreach ($props as $propName => $propValue) {
            // Получение ID свойства по коду свойства
            $dbOrderProps = CSaleOrderProps::GetList(
                array(),
                array("PERSON_TYPE_ID" => $payerType, "CODE" => $propName),
                false,
                false,
                array("ID")
            );
            ...
            if ($arOrderProps = $dbOrderProps->Fetch()) {
                $propId = $arOrderProps["ID"];
                // Добавление свойства профиля
                $propFields = [
                ...
                CSaleOrderUserPropsValue::Add($propFields);
            }
        }
    }
}
